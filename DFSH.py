class LMBB(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size,
                 stride=1, padding=None, dilation=1, groups=1,
                 internal_channels_1x1_3x3=None,
                 deploy=False, single_init=False):
        super(LMBB, self).__init__()
        self.deploy = deploy
...
class Detect_DFSH(Detect_LSCD):
    def __init__(self, nc=80, hidc=256, ch=()):
        super().__init__(nc, hidc, ch)
        self.conv = nn.ModuleList(nn.Sequential(LMBB(x, hidc, 3)) for x in ch)
...
class Segment_DFSH(Detect_DFSH):
    ...
    def __init__(self, nc=80, nm=32, npr=256, hidc=256, ch=()):
        ...
        self.detect = Detect_DFSH.forward
